...
- AI 向量化：阿里云百炼 Embedding API（`text-embedding-v3`，用于改写 evidence 语义校验）
- PDF 读取：pdf.js（CDN 或本地）
- PDF 导出：html2pdf.js（本地引入）
- 数据持久化：chrome.storage.local
- 图标库：Font Awesome 6（本地 CSS + webfonts，不使用 Kit JS 脚本）
...
```js
{
  ...
  profile: {
    ...
    projects: [{ name, company, role, period, highlights: [] }],  // 独立项目经历，company 与 experiences 中保持一致
    raw_text: "...",          // 原始简历文本，改写底稿
    segments: [               // 语义片段，改写溯源用
      { id: "exp_0_h_0", text: "..." },       // 工作经历片段
      { id: "project_0_h_0", text: "..." }    // 项目经历片段
    ]
  },
  ...
  // 投递记录
  applications: [
    { hash, company, title, applied_at, feedback: null | "replied" | "read_no_reply" | "no_response" }
    // feedback: null=未反馈, replied=收到回复, read_no_reply=已读不回, no_response=未收到消息
  ],
...
**API Key 存储**：DeepSeek Key 和百炼 Key 均直接硬编码在 `background.js` 顶部常量中。百炼 API 调用同样通过 `chrome.runtime.sendMessage` 触发，在 `background.js` 中统一发起。
